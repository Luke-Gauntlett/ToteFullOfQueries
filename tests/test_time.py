...
def test_time_no_file(aws_credentials):
    """Test that when no file exists, the function initializes with '0001-01-01'"""

    with mock_aws():
        client = boto3.client('s3')

        client.create_bucket(Bucket="testingBucket", CreateBucketConfiguration={"LocationConstraint": "eu-west-2"})
        ...
        file_content = client.get_object(Bucket="testingBucket", Key="last_extraction_times.json")
...
def test_last_time_is_mock_time(mock_datetime, aws_credentials):
    ...
    with mock_aws():
        ...
        mock_datetime.now.return_value = datetime(2003, 3, 3, 12, 15, 49, 000000)
        ...
        assert len(times) == 4
        assert times == ["0001-01-01 00:00:00.000000",str(faketime),str(faketime),str(faketime2)] 
        print(times)
