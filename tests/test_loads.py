...
def temp_db():
    ...
    # engine = create_engine("postgresql+pg8000://postgres@localhost/postgres")
...
class TestWarehouse:
    ...
    def test_appending_same_data_to_existing_table_in_warehouse_does____remove_duplicates(self, temp_db):
        test_df = pd.DataFrame(
            [
                {
                    "design_id": 1,
    ...
    def test_appending_same_data_to_existing_table_in_warehouse_does____(self):
        test_df = pd.DataFrame(
            [
                {                    
                    "design_id": 1,                  
        ...
        # print('p3')
        # result = conn.execute("""SELECT * FROM dim_design""")
        # print('p4')
    ...
    def test_data_appends_to_fact_sales_order_in_warehouse_with_incrementing_index(self, temp_db):
        test_df = pd.DataFrame(
            [
                {
                    "design_id": 1,
        ...
        cur.execute("""CREATE TABLE fact_sales_order (id integer primary key, design_id INT, created_at TEXT, design_name TEXT)""")

        
        load_df_to_warehouse(test_df, 'fact_sales_order', engine_conn=engine_conn)
        load_df_to_warehouse(test_df_2, 'fact_sales_order', engine_conn=engine_conn)
        result = cur.execute("SELECT * FROM fact_sales_order")
        results_list = result.fetchall()
        pprint(results_list)
        assert results_list == [(1, 1, '2022', 'Wooden'), (2, 2, '2023', 'Bronze'), (3, 3, '2023', 'Bronze'), (4, 4, '2023', 'W')]